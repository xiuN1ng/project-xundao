...
function seedSkillsData() {
  try {
    ...
    if (count && count.count > 0) return;
  } catch(e) { /* table doesn't exist yet, continue */ }
...
const TRIBULATION_TYPES = {
  // 五行天劫
  metal: { id: 'metal', name: '金劫', element: 'metal', weakness: 'wood', description: '金属性雷劫,以木属性克制', base_success_rate: 0.8, min_realm: 0 },
  wood: { id: 'wood', name: '木劫', element: 'wood', weakness: 'metal', description: '木属性雷劫,以金属性克制', base_success_rate: 0.8, min_realm: 1 },
  water: { id: 'water', name: '水劫', element: 'water', weakness: 'fire', description: '水属性雷劫,以火属性克制', base_success_rate: 0.7, min_realm: 2 },
  fire: { id: 'fire', name: '火劫', element: 'fire', weakness: 'water', description: '火属性雷劫,以水属性克制', base_success_rate: 0.6, min_realm: 3 },
  earth: { id: 'earth', name: '土劫', element: 'earth', weakness: 'wood', description: '土属性雷劫,以木属性克制', base_success_rate: 0.5, min_realm: 4 },
  // 特殊天劫
  heart_demon: { id: 'heart_demon', name: '心魔劫', element: 'mental', weakness: 'willpower', description: '考验道心,需击败心魔', base_success_rate: 0.7, min_realm: 1, special: true },
  celestial: { id: 'celestial', name: '九天雷劫', element: 'thunder', weakness: 'none', description: '九道天雷连续降落', base_success_rate: 0.2, min_realm: 5, special: true },
  chaos: { id: 'chaos', name: '混沌天劫', element: 'chaos', weakness: 'none', description: '最强天劫,需全服援助', base_success_rate: 0.1, min_realm: 7, special: true }
};

// 境界配置
const REALMS = [
  { level: 0, name: '炼气期', next: '筑基期', required_spirit: 0, icon: '💨' },
  { level: 1, name: '筑基期', next: '金丹期', required_spirit: 5000, icon: '🧱' },
  { level: 2, name: '金丹期', next: '元婴期', required_spirit: 20000, icon: '⚪' },
  { level: 3, name: '元婴期', next: '化神期', required_spirit: 80000, icon: '👶' },
  { level: 4, name: '化神期', next: '炼虚期', required_spirit: 300000, icon: '🧘' },
  { level: 5, name: '炼虚期', next: '合体期', required_spirit: 1000000, icon: '🌟' },
  { level: 6, name: '合体期', next: '大乘期', required_spirit: 5000000, icon: '☀️' },
  { level: 7, name: '大乘期', next: '飞升', required_spirit: 20000000, icon: '🏔️' },
  { level: 8, name: '飞升期', next: null, required_spirit: 0, icon: '✨' }
];

// 难度配置
const DIFFICULTY_CONFIG = {
  normal: { name: '普通', success_rate_modifier: 0, penalty: 'light', reward_multiplier: 1.0 },
  hard: { name: '困难', success_rate_modifier: -0.2, penalty: 'medium', reward_multiplier: 1.3 },
  nightmare: { name: '噩梦', success_rate_modifier: -0.4, penalty: 'heavy', reward_multiplier: 1.6 },
  legendary: { name: '传说', success_rate_modifier: -0.6, penalty: 'extreme', reward_multiplier: 2.0 }
};
...
const VIP_CONFIG = {
  1: {
    level: 1,
    name: 'VIP1',
    daily_sign_bonus: 0.20,
    idle_earn_bonus: 0.20,
    enhance_bonus: 0.05,
    price: 5000,
    duration_days: 30
  },
  2: {
    level: 2,
    name: 'VIP2',
    daily_sign_bonus: 0.50,
    idle_earn_bonus: 0.50,
    enhance_bonus: 0.10,
    price: 15000,
    duration_days: 30
  },
  3: {
    level: 3,
    name: 'VIP3',
    daily_sign_bonus: 1.00,
    idle_earn_bonus: 1.00,
    enhance_bonus: 0.20,
    price: 50000,
    duration_days: 30
  }
...
const PROTECTION_ITEMS = {
  tribulation_pill: { id: 'tribulation_pill', name: '渡劫丹', success_bonus: 0.15, price: 2000, rarity: 3 },
  superior_tribulation_pill: { id: 'superior_tribulation_pill', name: '高级渡劫丹', success_bonus: 0.25, price: 8000, rarity: 4 },
  perfect_tribulation_pill: { id: 'perfect_tribulation_pill', name: '完美渡劫丹', success_bonus: 0.35, price: 30000, rarity: 5 },
  substitute_charm: { id: 'substitute_charm', name: '替身符', dodge_one_hit: true, price: 1000, rarity: 2 },
  elixir_of_immortality: { id: 'elixir_of_immortality', name: '九转还魂丹', revive_once: true, price: 5000, rarity: 4 },
  spirit_bottle: { id: 'spirit_bottle', name: '灵气瓶', restore_spirit: 0.3, price: 500, rarity: 1 },
  divine_shield: { id: 'divine_shield', name: '护体神光', invincibility: 3, price: 10000, rarity: 4 }
};

// 获取天劫类型列表
app.get('/api/tribulation/types', (req, res) => {
  try {
    const { realm_level } = req.query;
    const realm = parseInt(realm_level) || 0;

    // 根据玩家境界过滤可用的天劫类型
    const availableTypes = Object.values(TRIBULATION_TYPES).filter(t => t.min_realm <= realm);
...
app.post('/api/tribulation/attempt', (req, res) => {
  try {
    ...
    const currentRealm = REALMS[player.realm_level] || REALMS[0];
    ...
    if (player.realm_level < tribType.min_realm) {
      return res.status(400).json({ success: false, error: `需要境界达到 ${REALMS[tribType.min_realm].name} 才能渡此天劫` });
    }
    ...
    // 计算基础成功率
    let baseSuccessRate = tribType.base_success_rate;

    // 根据境界计算成功率(境界越高,成功率越低)
    const realmBonus = 0.1 - (player.realm_level * 0.05);
    baseSuccessRate = Math.max(0.1, baseSuccessRate + realmBonus);

    // 检查道具加成
    let bonusSuccessRate = 0;
    const usedItems = [];

    if (use_items && Array.isArray(use_items)) {
      for (const itemId of use_items) {
        const item = PROTECTION_ITEMS[itemId];
        if (item && item.success_bonus) {
          // 检查玩家是否有此道具
          const playerItem = db.prepare('SELECT * FROM player_protection_items WHERE player_id = ? AND item_id = ?').get(actualPlayerId, itemId);
          if (playerItem && playerItem.quantity > 0) {
            bonusSuccessRate += item.success_bonus;
            usedItems.push(itemId);

            // 消耗道具
            db.prepare('UPDATE player_protection_items SET quantity = quantity - 1 WHERE player_id = ? AND item_id = ?').run(actualPlayerId, itemId);
            db.prepare('DELETE FROM player_protection_items WHERE quantity <= 0').run();
          }
        }
      }
    }
    ...
    // 应用难度修正
    const difficultyPenalty = diffConfig.success_rate_modifier;

    // 最终成功率
    const finalSuccessRate = Math.min(1, Math.max(0, baseSuccessRate + bonusSuccessRate + difficultyPenalty));
    ...
    // 计算奖励
    const rewardMultiplier = success ? diffConfig.reward_multiplier : 0.2;
    const spiritReward = currentRealm.required_spirit * rewardMultiplier;
    const stoneReward = Math.floor(currentRealm.required_spirit / 10 * rewardMultiplier);
    ...
    `).run(
      ...
      finalSuccessRate,
      success ? 'success' : 'failed',
      JSON.stringify({ spirit: spiritReward, stones: stoneReward })
    );

    // 更新玩家数据
    if (success) {
      db.prepare('UPDATE player SET realm_level = realm_level + 1, spirit_stones = spirit_stones + ? WHERE id = ?').run(stoneReward, actualPlayerId);
      // 更新战力
      try { combatPowerSystem?.calculateCombatPower?.({ id: actualPlayerId }); } catch (e) {}
    } else {
      // 失败惩罚:损失30%当前境界所需灵气
      const penaltySpirit = Math.floor(currentRealm.required_spirit * 0.3);
      db.prepare('UPDATE player SET spirit_stones = spirit_stones - ? WHERE id = ?').run(Math.max(0, penaltySpirit), actualPlayerId);
    }
    ...
    res.json({
      ...
      data: {
        ...
        bonus_success_rate: bonusSuccessRate,
        final_success_rate: finalSuccessRate,
        roll: roll,
        used_items: usedItems,
        rewards: success ? { spirit: spiritReward, stones: stoneReward } : null,
        penalty: success ? null : { lost_stones: Math.floor(currentRealm.required_spirit * 0.3) }
      }
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
...
// 获取渡劫状态
app.get('/api/tribulation/status', (req, res) => {
  try {
    const { player_id, record_id } = req.query;
    ...
    let records = [];
    if (record_id) {
      records = db.prepare('SELECT * FROM tribulation_records WHERE id = ?').all(record_id);
    } else {
      records = db.prepare('SELECT * FROM tribulation_records WHERE player_id = ? ORDER BY created_at DESC LIMIT 10').all(actualPlayerId);
    }

    // 获取玩家保护道具
    const protectionItems = db.prepare('SELECT * FROM player_protection_items WHERE player_id = ?').all(actualPlayerId);

    // 计算下一次渡劫的成功率预览
    let previewSuccessRate = 0.5;
    if (canAdvance) {
      const tribType = TRIBULATION_TYPES[REALMS[player.realm_level].name.replace('期', '') + '劫'] || TRIBULATION_TYPES.water;
      previewSuccessRate = tribType.base_success_rate + (0.1 - player.realm_level * 0.05);
    }
    ...
    res.json({
      ...
      data: {
        ...
        preview: {
          base_success_rate: Math.max(0.1, previewSuccessRate),
          required_spirit: currentRealm.required_spirit
        },
        ...
        protection_items: protectionItems
      }
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
...
// 使用保护道具
app.post('/api/tribulation/protect', (req, res) => {
  try {
    ...
    if (action === 'buy') {
      ...
      db.prepare('UPDATE player SET spirit_stones = spirit_stones - ? WHERE id = ?').run(item.price, actualPlayerId);

      // 添加或更新道具
      const existingItem = db.prepare('SELECT * FROM player_protection_items WHERE player_id = ? AND item_id = ?').get(actualPlayerId, item_id);
      if (existingItem) {
        db.prepare('UPDATE player_protection_items SET quantity = quantity + 1, updated_at = CURRENT_TIMESTAMP WHERE player_id = ? AND item_id = ?').run(actualPlayerId, item_id);
      } else {
        db.prepare('INSERT INTO player_protection_items (player_id, item_id, item_name, quantity) VALUES (?, ?, ?, ?)').run(actualPlayerId, item_id, item.name, 1);
      }
      ...
      res.json({
        ...
        data: {
          ...
          item_name: item.name,
          price: item.price,
          remaining_spirit_stones: player.spirit_stones - item.price
        }
      });
    } else if (action === 'use') {
      // 使用道具(检查是否拥有)
      const playerItem = db.prepare('SELECT * FROM player_protection_items WHERE player_id = ? AND item_id = ?').get(actualPlayerId, item_id);
...
try {
  ...
  // 渡劫系统
  try {
    const tribulationRoute = require('./backend/routes/tribulation');
    app.use('/api/tribulation', tribulationRoute);
    Logger.info('✅ 渡劫系统路由已加载 (backend/routes/tribulation.js)');
  } catch (e) {
