...
const REALM_DUNGEON_DATA = {
  ...
  '筑基期': {
    ...
    floors: [
      ...
      { id: 3, name: '雷霆深渊', enemy: '雷蛟', hp: 4500, atk: 250, def: 60, exp: 900, stones: 400, items: ['thunder_core'] },
    ...
    clearRewards: { exp: 8000, stones: 5000, items: ['realm_token_foundation', 'golden_core'] }
  },
  // 金丹期副本
  '金丹期': {
    name: '金丹期劫难',
    ...
    floors: [
      ...
      { id: 5, name: '渡劫台', enemy: '天劫化身', hp: 80000, atk: 2200, def: 500, exp: 15000, stones: 12000, items: ['lightning_core'] }
    ],
    boss: { name: '天劫化身', hp: 80000, atk: 2200, def: 500, rewards: { exp: 50000, stones: 30000, items: ['realm_token_golden'] } },
    clearRewards: { exp: 80000, stones: 50000, items: ['realm_token_golden', 'golden_core'] }
  },
  // 元婴期副本
  '元婴期': {
    name: '元婴期历练',
    ...
    clearRewards: { exp: 800000, stones: 500000, items: ['realm_token_nascent', 'divine_core'] }
  },
  // 化神期副本
  '化神期': {
    name: '化神期飞升关',
    ...
    floors: [
      ...
      { id: 5, name: '大道之门', enemy: '天道意志', hp: 4000000, atk: 80000, def: 20000, exp: 1000000, stones: 800000, items: ['heaven_core'] }
    ],
    boss: { name: '天道意志', hp: 4000000, atk: 80000, def: 20000, rewards: { exp: 5000000, stones: 3000000, items: ['realm_token_transcendent'] } },
    clearRewards: { exp: 8000000, stones: 5000000, items: ['realm_token_transcendent', 'heaven_core'] }
  },
  // 炼虚期副本
  '炼虚期': {
    name: '炼虚期合体',
    ...
    floors: [
      ...
      { id: 2, name: '万界战场', enemy: '万界主宰', hp: 10000000, atk: 150000, def: 45000, exp: 2000000, stones: 1500000, items: ['universe_core'] },
      { id: 3, name: '时间长河', enemy: '时间巨兽', hp: 15000000, atk: 220000, def: 60000, exp: 3500000, stones: 2500000, items: ['time_crystal'] },
      { id: 4, name: '空间风暴', enemy: '空间毁灭者', hp: 22000000, atk: 300000, def: 80000, exp: 5000000, stones: 4000000, items: ['space_core'] },
      { id: 5, name: '合道之巅', enemy: '大道化身', hp: 35000000, atk: 400000, def: 100000, exp: 8000000, stones: 6000000, items: ['unity_core'] }
    ],
    boss: { name: '大道化身', hp: 35000000, atk: 400000, def: 100000, rewards: { exp: 50000000, stones: 30000000, items: ['realm_token_void'] } },
    clearRewards: { exp: 80000000, stones: 50000000, items: ['realm_token_void', 'unity_core'] }
  },
  // 合体期副本
  '合体期': {
    name: '合体期天劫',
    ...
    floors: [
      ...
      { id: 4, name: '因果之轮', enemy: '因果律', hp: 180000000, atk: 1400000, def: 350000, exp: 50000000, stones: 40000000, items: ['karma_core'] },
      { id: 5, name: '天道轮回', enemy: '轮回主宰', hp: 280000000, atk: 2000000, def: 450000, exp: 80000000, stones: 60000000, items: ['reincarnation_core'] }
    ],
    boss: { name: '轮回主宰', hp: 280000000, atk: 2000000, def: 450000, rewards: { exp: 500000000, stones: 300000000, items: ['realm_token_divine'] } },
    clearRewards: { exp: 800000000, stones: 500000000, items: ['realm_token_divine', 'reincarnation_core'] }
  },
  // 大乘期副本
  '大乘期': {
    name: '大乘期飞升',
    ...
    floors: [
      ...
      { id: 4, name: '大道尽头', enemy: '大道守护', hp: 1500000000, atk: 9000000, def: 2000000, exp: 600000000, stones: 500000000, items: ['ultimate_core'] },
  ...
  '渡劫期': {
    ...
    floors: [
      ...
      { id: 4, name: '因果终极', enemy: '因果创世神', hp: 18000000000, atk: 60000000, def: 15000000, exp: 10000000000, stones: 8000000000, items: ['creation_core'] },
      { id: 5, name: '成就真仙', enemy: '天道化身', hp: 30000000000, atk: 80000000, def: 20000000, exp: 20000000000, stones: 15000000000, items: ['true_god_core'] }
    ],
    ...
    clearRewards: { exp: 150000000000, stones: 80000000000, items: ['realm_token_tribulation', 'true_god_core'] }
  }
};

// 副本状态
const realmDungeonState = {
...
function startRealmDungeonBattle() {
  ...
  return {
    ...
    isBoss: realmDungeonState.currentFloor > dungeonData.floors.length || realmDungeonState.currentFloor === dungeonData.floors.length + 1
  };
}
...
// ==================== 扫荡功能 ====================
function sweepRealmDungeon(realmId, floors) {
  const player = gameState.player;
  const progress = realmDungeonState.progress[realmId];
  
  if (!progress || progress.highestFloor < 1) {
    return { success: false, message: '尚未通关任何关卡' };
  }
  
  const sweepFloors = Math.min(floors || progress.highestFloor, progress.highestFloor);
  const dungeonData = REALM_DUNGEON_DATA[realmId];
  
  if (!dungeonData) {
    return { success: false, message: '境界副本不存在' };
  }
  
  // 计算玩家属性
  const playerStats = game.calculatePlayerStats();
  
  let totalExp = 0;
  let totalStones = 0;
  let totalSpirit = 0;
  
  for (let floor = 1; floor <= sweepFloors; floor++) {
    const floorData = dungeonData.floors[floor - 1];
    if (!floorData) continue;
    
    // 动态难度：敌人属性基于玩家属性
    const difficultyMultiplier = Math.pow(1.1, floor - 1);
    const enemyHp = Math.floor(playerStats.hp * difficultyMultiplier * 0.5);
    const enemyAtk = Math.floor(playerStats.atk * difficultyMultiplier * 0.3);
    
    // 扫荡奖励 = 正常奖励 × 80%
    const expReward = Math.floor(floorData.exp * 0.8);
    const stoneReward = Math.floor(floorData.stones * 0.8);
    const spiritReward = Math.floor(100 * difficultyMultiplier * 0.8); // 灵气奖励
    
    totalExp += expReward;
    totalStones += stoneReward;
    totalSpirit += spiritReward;
  }
  
  // 发放奖励
  player.exp = (player.exp || 0) + totalExp;
  player.spiritStones = (player.spiritStones || 0) + totalStones;
  player.spirit = (player.spirit || 0) + totalSpirit;
...
// ==================== 动态难度计算 ====================
function calculateDynamicEnemyStats(floor, playerStats) {
  // 每层增加10%难度
  const difficultyMultiplier = Math.pow(1.1, floor - 1);
  
  // HP: 玩家HP的0.5~2倍
  const hpRatio = 0.5 + (floor * 0.15);
  
  // ATK: 玩家攻击的0.3~1倍
  const atkRatio = 0.3 + (floor * 0.07);
  
  // DEF: 玩家防御的0.2倍
  const defRatio = 0.2;
  
  return {
    hp: Math.floor(playerStats.hp * Math.min(hpRatio, 2)),
    atk: Math.floor(playerStats.atk * Math.min(atkRatio, 1)),
...
module.exports = {
  REALM_DUNGEON_DATA,
  realmDungeonState,
  getAvailableRealmDungeons,
  enterRealmDungeon,
  startRealmDungeonBattle,
  attackRealmDungeonEnemy,
  exitRealmDungeon,
  getRealmDungeonStatus,
  initRealmDungeonData,
  saveRealmDungeonData,
  sweepRealmDungeon,
  calculateDynamicEnemyStats
};
