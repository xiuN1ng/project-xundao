...
// 副本配置
const dungeons = [
  { id: 1, icon: '🗿', name: '新手副本', reqLevel: 1, progress: 100, unlocked: true, cost: 0, dropChance: 0.3, drops: [{itemId: 101, name: '灵气丹', icon: '🧪', count: 1}] },
...
router.post('/battle/:battleId', (req, res) => {
  ...
  if (won) {
    ...
    if (Math.random() < dungeon.dropChance) {
      const drop = dungeon.drops[Math.floor(Math.random() * dungeon.drops.length)];
      drops = [drop];
