...
function calculateBeastStats(beastIndex) {
  ...
  return {
    atk: Math.floor((data.base_atk + (beast.level - 1) * data.growth_atk) * affectionData.bonus * moodBonus),
    hp: Math.floor((data.base_hp + (beast.level - 1) * data.growth_hp) * affectionData.bonus * moodBonus),
    level: beast.level,
...
console.log('🦊 灵兽系统 v4.0 (API版) 已加载');
