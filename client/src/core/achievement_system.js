...
const ACHIEVEMENT_MILESTONES = {
  ...
  'achievement_10': {
    ...
    type: 'count',
    target: 10,
    rewards: { spiritStones: 500, title: '成就达人' },
    icon: '🥉'
  },
  'achievement_30': {
    id: 'achievement_30',
    ...
    type: 'count',
    target: 30,
    rewards: { spiritStones: 1500, atkBonus: 0.05 },
    icon: '🥈'
  },
  'achievement_50': {
    id: 'achievement_50',
    ...
    type: 'count',
    target: 50,
    rewards: { spiritStones: 3000, atkBonus: 0.1, defBonus: 0.1 },
    icon: '🥇'
  },
  'achievement_80': {
    id: 'achievement_80',
    ...
    type: 'count',
    target: 80,
    rewards: { spiritStones: 5000, spiritRateBonus: 0.2 },
    icon: '🏆'
  },
  'achievement_100': {
    id: 'achievement_100',
    ...
    type: 'count',
    target: 100,
    rewards: { spiritStones: 10000, title: '千古第一人', allBonus: 0.1 },
    icon: '👑'
  },
  ...
  'collect_50': {
    ...
    type: 'collect',
    target: 50,
    rewards: { spiritStones: 300, dropBonus: 0.1 },
    icon: '📦'
  },
  'collect_100': {
    id: 'collect_100',
    ...
    type: 'collect',
    target: 100,
    rewards: { spiritStones: 800, dropBonus: 0.2 },
    icon: '🎁'
  },
  ...
  'combat_100': {
    ...
    type: 'combat',
    target: 100,
    rewards: { spiritStones: 500, atkBonus: 0.1 },
    icon: '⚔️'
  },
  'combat_500': {
    id: 'combat_500',
    ...
    type: 'combat',
    target: 500,
    rewards: { spiritStones: 2000, atkBonus: 0.15 },
    icon: '🗡️'
  },
  ...
  'wealth_100k': {
    ...
    type: 'wealth',
    target: 100000,
    rewards: { spiritStones: 1000, stoneBonus: 0.1 },
    icon: '💰'
  },
  'wealth_1m': {
    id: 'wealth_1m',
    ...
    type: 'wealth',
    target: 1000000,
    rewards: { spiritStones: 5000, stoneBonus: 0.2 },
    icon: '💎'
  },
  ...
  'realm_5': {
    ...
    type: 'realm',
    target: 5,
    rewards: { spiritStones: 2000, realmExpBonus: 0.2 },
    icon: '🧘'
  },
  'realm_10': {
    id: 'realm_10',
    ...
    type: 'realm',
    target: 10,
    rewards: { spiritStones: 5000, realmExpBonus: 0.3, title: '得道高人' },
    icon: '🧘‍♂️'
  }
};
...
function checkMilestoneProgress() {
  ...
  for (const [id, milestone] of Object.entries(ACHIEVEMENT_MILESTONES)) {
    if (!milestones[id]) {
      milestones[id] = { progress: 0, claimed: false };
    }
    
    let currentProgress = 0;
    switch (milestone.type) {
      case 'count':
        currentProgress = achievementCount;
        break;
      case 'collect':
        currentProgress = player.totalEquipmentCollected || 0;
        break;
      case 'combat':
        currentProgress = (gameState.stats?.wins || 0);
        break;
      case 'wealth':
        currentProgress = Math.max(player.spiritStones || 0, player.totalSpiritStonesEarned || 0);
        break;
      case 'realm':
        currentProgress = player.realmLevel || 0;
        break;
    }
    
    milestones[id].progress = Math.min(currentProgress, milestone.target);
  }
}
...
function claimMilestoneReward(milestoneId) {
  const player = gameState.player;
  const milestones = getMilestoneData();
  const milestone = ACHIEVEMENT_MILESTONES[milestoneId];
  ...
  if (!milestones[milestoneId]) {
    return { success: false, message: '进度未初始化' };
  }
  
  if (milestones[milestoneId].claimed) {
    return { success: false, message: '奖励已领取' };
  }
  
  if (milestones[milestoneId].progress < milestone.target) {
    return { success: false, message: '未达成条件' };
  }
  
  // 发放奖励
  const rewards = milestone.rewards;
  if (rewards.spiritStones) {
    player.spiritStones += rewards.spiritStones;
  }
  if (rewards.title) {
  ...
  if (rewards.atkBonus) {
    achievementSystem.bonuses.atkBonus = (achievementSystem.bonuses.atkBonus || 0) + rewards.atkBonus;
  }
  if (rewards.defBonus) {
    achievementSystem.bonuses.defBonus = (achievementSystem.bonuses.defBonus || 0) + rewards.defBonus;
  }
  if (rewards.spiritRateBonus) {
    achievementSystem.bonuses.spiritRateBonus = (achievementSystem.bonuses.spiritRateBonus || 0) + rewards.spiritRateBonus;
  }
  if (rewards.dropBonus) {
    achievementSystem.bonuses.dropBonus = (achievementSystem.bonuses.dropBonus || 0) + rewards.dropBonus;
  }
  if (rewards.stoneBonus) {
    achievementSystem.bonuses.stoneBonus = (achievementSystem.bonuses.stoneBonus || 0) + rewards.stoneBonus;
  }
  if (rewards.realmExpBonus) {
    achievementSystem.bonuses.realmExpBonus = (achievementSystem.bonuses.realmExpBonus || 0) + rewards.realmExpBonus;
  }
  if (rewards.allBonus) {
    achievementSystem.bonuses.atkBonus = (achievementSystem.bonuses.atkBonus || 0) + rewards.allBonus;
    achievementSystem.bonuses.defBonus = (achievementSystem.bonuses.defBonus || 0) + rewards.allBonus;
  }
  
  milestones[milestoneId].claimed = true;
  
  return { 
    success: true, 
    message: `领取${milestone.name}奖励成功！` 
  };
}
...
window.ACHIEVEMENT_MILESTONES = ACHIEVEMENT_MILESTONES;
window.getMilestoneData = getMilestoneData;
window.getUnlockedAchievementCount = getUnlockedAchievementCount;
window.checkMilestoneProgress = checkMilestoneProgress;
