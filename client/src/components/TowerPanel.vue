  ...
  <div class="tower-panel">
    ...
    <div class="tower-header">
      <div class="tower-title">
        ...
        <span class="title-text">无尽塔</span>
        <span class="tower-subtitle">挑战极限·登顶为尊</span>
      </div>
    ...
    <div v-if="activeMode === 'challenge'" class="mode-content">
      ...
      <div class="enemy-section">
        ...
        <!-- 战斗 Arena -->
        <div class="battle-arena" :class="{ 'boss-mode': isBossFloor }">
          <!-- 玩家侧 -->
          <div class="combatant player-side" :class="{ attacking: isAttacking && attacker === 'player' }">
            <div class="combatant-avatar player-avatar">
              ...
              <div class="hp-bar-container">
                <div class="hp-bar player-hp" :style="{ width: (playerHp / playerMaxHp * 100) + '%' }"></div>
              </div>
            ...
            <div class="damage-numbers">
              <div 
                v-for="(dmg, i) in playerDamageLog" 
                :key="i"
                class="damage-pop player-damage"
          ...
          <div class="vs-zone">
            ...
            <div class="battle-round" v-if="battleActive">
              第 {{ battleRound }} 回合
            </div>
          </div>
          ...
          <div class="combatant enemy-side" :class="{ attacking: isAttacking && attacker === 'enemy' }">
            <div class="combatant-avatar enemy-avatar" :class="{ 'boss-avatar': isBossFloor }">
              ...
              <div class="hp-bar-container">
                <div class="hp-bar enemy-hp" :style="{ width: (enemyHp / enemyMaxHp * 100) + '%' }"></div>
              </div>
            ...
            <div class="damage-numbers">
              <div 
                v-for="(dmg, i) in enemyDamageLog" 
                :key="i"
                class="damage-pop enemy-damage"
                :style="{ animationDelay: (i * 0.3) + 's' }"
              >
                -{{ dmg }}
              </div>
        ...
        <div class="battle-actions" v-if="!battleActive && !battleEnded">
          <button class="btn btn-attack" @click="startBattle" :disabled="currentFloor > maxFloor + 1">
            ⚔️ 开始挑战
          </button>
        </div>
        ...
        <div class="battle-actions" v-else-if="battleEnded">
          <div class="battle-result" :class="battleWin ? 'win' : 'lose'">
            ...
            <div class="result-btns">
              <button class="btn btn-sm" @click="nextFloor" v-if="battleWin">下一层 →</button>
              <button class="btn btn-sm btn-secondary" @click="retryBattle" v-if="!battleWin">重试</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="floor-selector">
        <div class="selector-header">
          ...
          <div class="floor-range">
            <button class="range-btn" @click="scrollFloors(-5)">◀◀</button>
    ...
    <div v-if="activeMode === 'sweep'" class="mode-content">
      ...
      <div class="sweep-config">
        ...
        <div class="sweep-range">
          <div class="range-row">
            ...
            <div class="range-input-group">
              <button class="input-btn" @click="sweepStartFloor = Math.max(1, sweepStartFloor - 1)">-</button>
              <span class="range-val">{{ sweepStartFloor }}</span>
              <button class="input-btn" @click="sweepStartFloor++">+</button>
            </div>
          </div>
          <div class="range-row">
            <span class="range-label">结束层</span>
            <div class="range-input-group">
              <button class="input-btn" @click="sweepEndFloor = Math.max(sweepStartFloor, sweepEndFloor - 1)">-</button>
              <span class="range-val">{{ sweepEndFloor }}</span>
              <button class="input-btn" @click="sweepEndFloor++">+</button>
            </div>
          </div>
        </div>
        ...
        <div class="sweep-info">
          <div class="sweep-calc">
            ...
            消耗 <span class="highlight">无</span>
            <span class="vline">|</span>
        ...
        <button 
          class="btn btn-sweep"
          @click="startSweep"
          :disabled="sweeping || sweepEndFloor < sweepStartFloor || todaySweep >= maxSweepTimes"
        >
          <span v-if="!sweeping">🧹 开始扫荡</span>
          <span v-else>⏳ 扫荡中... {{ sweepProgress }}/{{ sweepTotal }}</span>
        </button>
      </div>
      ...
      <div class="sweep-progress" v-if="sweeping">
        <div class="progress-header">
          <span>正在扫荡第 {{ sweepCurrentFloor }} 层</span>
          <span class="progress-pct">{{ sweepTotal > 0 ? Math.round((sweepProgress / sweepTotal) * 100) : 0 }}%</span>
        </div>
        <div class="progress-bar">
          <div class="progress-fill" :style="{ width: (sweepTotal > 0 ? sweepProgress / sweepTotal * 100 : 0) + '%' }"></div>
        </div>
        <div class="sweep-log">
          <div 
    ...
    <div v-if="activeMode === 'rank'" class="mode-content">
      <div class="rank-section">
        <div class="rank-header">
          <span class="rank-title">🏆 无尽塔排行榜</span>
        </div>
      ...
      <div class="rewards-section">
        <div class="rewards-header">
          ...
          <div class="floor-filter">
            <button 
              ...
              :key="type.id"
              class="filter-btn"
              :class="{ active: selectedRewardType === type.id }"
        ...
        <div class="rewards-grid">
          ...
          >
            ...
            <div class="rf-items">
              <span v-for="item in floor.rewards" :key="item.icon" class="rf-item">
                {{ item.icon }} {{ item.val }}
              </span>
            </div>
          </div>
...
const playerDamageLog = ref([])
const enemyDamageLog = ref([])
...
function startBattle() {
  ...
  battleRound.value = 0
  playerDamageLog.value = []
  enemyDamageLog.value = []
...
function battleTick() {
  ...
  playerDamageLog.value.push(playerDmg)
  if (playerDamageLog.value.length > 3) playerDamageLog.value.shift()
  ...
  setTimeout(() => {
    ...
    enemyDamageLog.value.push(enemyDmg)
    if (enemyDamageLog.value.length > 3) enemyDamageLog.value.shift()
...
function simulateSweep() {
  ...
  sweepLogs.value.push({
    ...
    text: `第${floor}层 ${isBoss ? '👑BOSS' : '⚔️'} 获得灵气+${spiritReward} 经验+${expReward}`
  })
...
<style scoped>
.tower-panel {
  background: linear-gradient(rgba(15, 15, 40, 0.92), rgba(20, 15, 50, 0.95));
  border-radius: 16px;
  border: 2px solid rgba(102, 126, 234, 0.4);
  box-shadow: 0 8px 40px rgba(0, 0, 0, 0.6), 0 0 30px rgba(102, 126, 234, 0.15);
  color: #e2e8f0;
  max-height: 88vh;
  overflow: hidden;
  display: flex;
...
.vs-text.flash {
  animation: vsFlash 0.6s infinite;
}
...
.rank-floor .floor-num {
  font-size: 18px;
  font-weight: bold;
  color: #ffd700;
}

.rank-floor .floor-label {
  font-size: 11px;
  color: #888;
}
...
.reward-floor-card:hover {
  background: rgba(0, 0, 0, 0.35);
  transform: translateY(-2px);
}

.reward-floor-card.boss-floor {
