<template>
  <div class="intersect-war-panel" v-if="visible">
    ...
    <div class="panel-content">
      ...
      <div v-if="currentTab === 'battlefield'" class="tab-battlefield">
        ...
        <div class="my-guild-card" v-if="hasGuild">
          ...
          <button class="declare-btn" @click="switchTab('declare')" :disabled="!canDeclareWar">
...
const warStatus = ref('signup'); // signup, fighting, ended
...
const formationSlots = ref([
  { position: 0, unit: null },
  { position: 1, unit: null },
  { position: 2, unit: null },
  { position: 3, unit: null },
...
const warStateClass = computed(() => {
  switch (warStatus.value) {
    case 'signup': return 'banner-signup';
    case 'fighting': return 'banner-fighting';
    case 'ended': return 'banner-ended';
    default: return 'banner-signup';
  }
});

const warBannerIcon = computed(() => {
  switch (warStatus.value) {
    case 'signup': return '📢';
...
const warBannerTitle = computed(() => {
  switch (warStatus.value) {
    case 'signup': return '宣战阶段';
    case 'fighting': return '战斗阶段';
    case 'ended': return '赛季结束';
    default: return '宣战阶段';
  }
});

const warBannerSubtitle = computed(() => {
  switch (warStatus.value) {
    case 'signup': return '各仙盟可发起宣战';
    case 'fighting': return '战场争夺正在进行';
    case 'ended': return '赛季奖励发放中';
    default: return '';
  }
});

const warCountdownLabel = computed(() => {
  switch (warStatus.value) {
    case 'signup': return '后开始战斗';
    case 'fighting': return '后结束战斗';
    default: return '';
  }
});

const canDeclareWar = computed(() => {
  return warStatus.value === 'signup' && hasGuild.value;
});

const canDeclare = computed(() => {
  return selectedTarget.value && hasGuild.value && warStatus.value === 'signup';
});

const formationHint = computed(() => {
  const hints = {
    attack: '高攻击加成，适合速战速决',
    defense: '高防御加成，适合持久战',
    balance: '攻防均衡，适合常规战斗',
  ...
  return hints[currentFormation.value] || '';
});

const totalBattlePower = computed(() => {
  return membersList.value.reduce((sum, m) => sum + m.power, 0);
});
...
function selectZone(zone) {
  // 显示区域详情
  showMessage(`查看 ${zone.name} 战况`, 'info');
}

function selectFormation(type) {
  currentFormation.value = type.id;
}
...
function toggleUnitSelection(unit) {
  ...
  if (idx === -1) {
    if (selectedUnits.value.length < maxFormationUnits) {
      ...
      showMessage(`最多只能上阵 ${maxFormationUnits} 名弟子`, 'warning');
    }
  } else {
...
function confirmFormation() {
  ...
  selected.forEach((unit, idx) => {
    if (idx < formationSlots.value.length) {
      formationSlots.value[idx].unit = unit;
    }
  });
  formationUnits.value = selected;
  showMessage('布阵成功！', 'success');
  selectedUnits.value = [];
}

async function declareWar() {
  if (!canDeclare.value) return;

  try {
    const response = await fetch('/api/sect-war/declare', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        targetGuildId: selectedTarget.value.id,
        reason: declareReason.value
      })
    });
    ...
    if (response.ok) {
      ...
      if (data.success) {
        showMessage(`已向 ${selectedTarget.value.name} 宣战！`, 'success');
        declareHistory.value.unshift({
          id: Date.now(),
          icon: '⚔️',
          title: `向${selectedTarget.value.name}宣战`,
          time: Date.now(),
          status: 'pending'
        });
...
async function loadWarData() {
  try {
    const response = await fetch('/api/sect-war/info', {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' }
    });
    ...
    if (response.ok) {
      ...
      if (data.warStatus) warStatus.value = data.warStatus;
      if (data.zones) warZones.value = data.zones;
      if (data.ranking) battleRanking.value = data.ranking;
      if (data.myGuild) myGuild.value = { ...myGuild.value, ...data.myGuild };
      if (data.hasGuild !== undefined) hasGuild.value = data.hasGuild;
    }
  } catch (e) {
    console.log('使用默认跨服战数据');
  }
}

function startCountdown() {
  warCountdown.value = 3600; // 默认1小时
  countdownTimer = setInterval(() => {
    if (warCountdown.value > 0) {
      warCountdown.value--;
    }
  }, 1000);
}
...
function zoneStatusText(status) {
  const texts = {
    free: '可争夺',
    contested: '争夺中',
    occupied: '已占领'
...
.intersect-war-panel {
  ...
  width: 560px;
  max-height: 85vh;
  background: linear-gradient(rgba(15, 15, 35, 0.88), rgba(22, 33, 62, 0.92)), url('../assets/bg-sect-war.png') center/cover no-repeat;
  border: 2px solid #b8860b;
  animation: panelFadeIn 0.3s ease;
  border-radius: 16px;
  box-shadow: 0 10px 50px rgba(0, 0, 0, 0.6), 0 0 30px rgba(184, 134, 11, 0.2);
  z-index: 1000;
  overflow: hidden;
...
.war-banner.banner-signup {
  background: linear-gradient(90deg, rgba(251, 191, 36, 0.2), rgba(184, 134, 11, 0.2));
  border-bottom: 1px solid rgba(251, 191, 36, 0.3);
}

.war-banner.banner-fighting {
  background: linear-gradient(90deg, rgba(239, 68, 68, 0.2), rgba(220, 38, 38, 0.2));
  border-bottom: 1px solid rgba(239, 68, 68, 0.3);
  animation: battlePulse 2s infinite;
}

.war-banner.banner-ended {
...
.my-guild-card .guild-emblem {
  font-size: 40px;
  margin-right: 12px;
}

.my-guild-card .guild-details {
  flex: 1;
}

.my-guild-card .guild-name {
  font-size: 16px;
  color: white;
  font-weight: bold;
}

.my-guild-card .guild-rank,
.my-guild-card .guild-score {
  font-size: 12px;
  color: #a0a0b8;
}
...
.declare-btn:hover:not(:disabled) {
...
.map-zone.zone-contested {
  border-color: rgba(239, 68, 68, 0.5);
  animation: contestedPulse 1.5s infinite;
}
...
.zone-status-tag.status-free {
  background: rgba(128, 128, 128, 0.3);
  color: #aaa;
}

.zone-status-tag.status-contested {
  background: rgba(239, 68, 68, 0.3);
  color: #ef4444;
}

.zone-status-tag.status-occupied {
...
.guild-select-item:hover {
  border-color: rgba(184, 134, 11, 0.3);
}

.guild-select-item.selected {
  border-color: #ffd700;
...
.guild-select-item .guild-info {
  flex: 1;
}

.guild-select-item .guild-name {
  font-size: 13px;
  color: white;
}

.guild-select-item .guild-power {
  font-size: 11px;
  color: #888;
}
...
.declare-war-btn:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 6px 20px rgba(239, 68, 68, 0.4);
}

.declare-war-btn:disabled {
  background: #555;
  cursor: not-allowed;
}
...
.history-status.status-win {
  background: rgba(16, 185, 129, 0.2);
  color: #10b981;
}

.history-status.status-lose {
...
.unit-card .unit-level {
  font-size: 11px;
  color: #888;
}

.unit-card .unit-power {
  font-size: 11px;
}
...
.confirm-formation-btn:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 6px 20px rgba(16, 185, 129, 0.4);
}

.confirm-formation-btn:disabled {
  background: #555;
  cursor: not-allowed;
}
...
.member-status.status-ready {
  background: rgba(16, 185, 129, 0.2);
  color: #10b981;
}

.member-status.status-fighting {
  background: rgba(239, 68, 68, 0.2);
  color: #ef4444;
}

.member-status.status-rest {
